...
namespace Action
{
    public class CeruleanRightCorner : CeruleanActionBase
    {
        private int phase = 0;
        ...
        public CeruleanRightCorner(CeruleanController contr) : base(contr)
        {
        }
        ...
        public override void Update()
        {
            if (phase == 0)
            {
                if (!targetSelected)
                {
                    Vector2 final = new Vector2(31f, 16.1f);
            ...
            if (phase == 2)
            {
                ...
                SpawnRock(controller.transform.position + Vector3.left, targetThree);
                SpawnRock(controller.transform.position + Vector3.left + Vector3.down, targetTwo);
                SpawnRock(controller.transform.position + Vector3.left + Vector3.down + Vector3.down, targetOne);
            }
        }
    }
}